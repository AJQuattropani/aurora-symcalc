...
void test_build()
{
  ...
  tokens = parse_string_tokens(string2, strlen(string2));
  print(&tokens);
  
  oper = least_significant_operation(tokens.head, NULL);

  x = 2;
  y = a_evaluate(oper, x);
  
  y_exp = b_add(b_log(x,2), b_sub(b_log(2.718281828459045,2*u_sin(b_sub(x,1.5))), b_mult(b_pow(u_arctan(b_div(x,2)),b_sub(2,x)), x)));
  EQUALS(y, y_exp);
  a_free_recursive(oper);
  empty_list(&tokens);
