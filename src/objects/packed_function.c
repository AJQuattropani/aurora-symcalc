...
f_node *f_pack_duplicate(stack* stack, const f_node *ref) {
  ...
  switch(ref->ty) {
    case BINARY:
      dup = (f_node*)s_alloc(stack, offsetof(f_node, __align_data) + sizeof(struct binary_f));
      dup->bf.op = ref->bf.op;
      dup->bf.left = f_pack_duplicate(stack, ref->bf.left);
      dup->bf.right = f_pack_duplicate(stack, ref->bf.right);
    break;
    case UNARY:
      dup = (f_node*)s_alloc(stack, offsetof(f_node, __align_data) + sizeof(struct unary_f));
      dup->uf.op = ref->uf.op;
      dup->uf.in = f_pack_duplicate(stack, ref->uf.in);
    break;
    case CONSTANT:
      dup = (f_node*)s_alloc(stack, offsetof(f_node, __align_data) + sizeof(struct const_f));
      dup->cf.output = ref->cf.output;
    break;
    case IDENTITY:
      dup = (f_node*)s_alloc(stack, offsetof(f_node, __align_data) + sizeof(struct iden_f));
