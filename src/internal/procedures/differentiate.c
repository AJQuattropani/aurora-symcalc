...
f_node *diff_un(const f_node *in, vector_size_t argidx, f_attribs *attr) {
  ...
  if (vu_neg == in->uf.op) {
    ...
    *fg = (f_node){.name = m_from_cstr("cos"), 
      .ty = UNARY, .uf = {.op = vu_cos, .in = copy_fnode_recurse(in->uf.in)}};
    *fprime = (f_node){.name = m_from_cstr("*"),
      .ty = BINARY, .bf = {.op = vb_mul, .left = fg, .right = dg}};
    ...
    *fg = (f_node){.name = m_from_cstr("sin"),
      .ty = UNARY, .uf = {.op = vu_sin, .in = copy_fnode_recurse(in->uf.in)}};
    *mfg = (f_node){.name = m_from_cstr("--"), 
      .ty = UNARY, .uf = {.op = vu_neg, .in = fg}};
    }
    *fprime = (f_node){.name = m_from_cstr("*"), 
      .ty = BINARY, .bf = {.op = vb_mul, .left = mfg, .right = dg}};
  } else if (vu_tan == in->uf.op) {
    fprime = new_fnode();
    f_node *sec2g = new_fnode();
    {
      f_node *secg = new_fnode();
      *secg = (f_node){.name = m_from_cstr("sec"),
       .ty = UNARY, .uf = {.op = vu_sec, .in = copy_fnode_recurse(in->uf.in)}};
      f_node *two = new_fnode();
      *two = (f_node){.name = m_from_cstr("2"), .ty = CONSTANT, .cf = {.output = make_scalar(2.0)}};
      *sec2g = (f_node){.name = m_from_cstr("^"),
       .ty = BINARY, .bf = {.op = vb_pow, .left = secg, .right = two}};
    }
    *fprime = (f_node){.name = m_from_cstr("*"),
      .ty = BINARY, .bf = {.op = vb_mul, .left = sec2g, .right = dg}};
  } else if (vu_sec == in->uf.op) {
    fprime = new_fnode();
    f_node *secgtang = new_fnode();
    {
      f_node *tang = new_fnode();
      *tang = (f_node){.name = m_from_cstr("tan"),
        .ty = UNARY, .uf = {.op = vu_tan, .in = copy_fnode_recurse(in->uf.in)}};
      *secgtang = (f_node){.name = m_from_cstr("*"),
        .ty = BINARY, .bf = {.op = vb_mul, .left = copy_fnode_recurse(in), .right = tang}};
    }
    *fprime = (f_node){.name = m_from_cstr("*"),
      .ty = BINARY, .bf = {.op = vb_mul, .left = secgtang, .right = dg}};
    ...
    f_node *mcscgcotg = new_fnode();
    {
      ...
      *cotg = (f_node){.name = m_from_cstr("cot"), 
          .ty = UNARY, .uf = {.op = vu_cot, .in = copy_fnode_recurse(in->uf.in)}};
      *cscgcotg = (f_node){.name = m_from_cstr("*"),
        .ty = BINARY, .bf = {.op = vb_mul, .left = copy_fnode_recurse(in), .right = cotg}};
      }
      *mcscgcotg = (f_node){.name = m_from_cstr("*"),
        .ty = UNARY, .uf = {.op = vu_neg, .in = cscgcotg}};
    }
    *fprime = (f_node){.name = m_from_cstr("*"),
        .ty = BINARY, .bf = {.op = vb_mul, .left = mcscgcotg, .right = dg}};
  } else if (vu_log == in->uf.op) {
    fprime = new_fnode();
    f_node *invg = new_fnode();
    {
    f_node *m1 = new_fnode();
    *m1 = (f_node){.name = m_from_cstr("-1"), .ty = CONSTANT, .cf = {.output = make_scalar(-1.0)}};
    *invg = (f_node){.name = m_from_cstr("^"),
      .ty = BINARY, .bf = {.op = vb_pow, .left = copy_fnode_recurse(in->uf.in), .right = m1}};
    }
    *fprime = (f_node){.name = m_from_cstr("*"), 
      .ty = BINARY, .bf = {.op = vb_mul, .left = invg, .right = dg}};
  } else {
    fprintf(stderr, "Failed to find matching function pointer in %s\n", __func__);
    exit(1);
  }
