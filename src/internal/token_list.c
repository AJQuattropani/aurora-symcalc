...
void empty_token_array(token_array * restrict arr) {
  memset(arr->data, 0, arr->capacity * sizeof(token));
  arr->size = 0;
}

void destroy_token_array(token_array * restrict arr) {
  free(arr->data);
  arr->data = NULL;
  arr->capacity = 0;
  arr->size = 0;
...
void push_token_back(token_array * restrict arr, const token * restrict tok) {
